...
import { getVehicleLocationLabel, hasValidGps } from '../../lib/location'
...
const status = (vehicle: Vehicle, gps?: GPS) => {
  if (vehicle.status === 'maintenance') return statusMeta.maintenance
  if (vehicle.status === 'delayed') return statusMeta.delayed
  if (gps && typeof gps.speed === 'number' && Number.isFinite(gps.speed) && gps.speed > 5) return statusMeta.in_transit
  return statusMeta[vehicle.status] || statusMeta.idle
}
...
const hasLiveGps = (gps?: GPS) => hasValidGps(gps)
const locationLabel = (vehicle: Vehicle, gps?: GPS) => getVehicleLocationLabel(vehicle, gps)
...
export const CarrierAssetsReference: React.FC<Props> = ({ vehicles, gpsData, canMutate, onAdd, onEdit, onDelete }) => {
  ...
  const filtered = vehicles.filter((vehicle) => {
    ...
    const liveStatus = status(vehicle, gps)
    return (!query || plateNumber(vehicle).toLowerCase().includes(query) || vehicle.vehicle_type.toLowerCase().includes(query) || (isPowered(vehicle) && (vehicle.driver_name || '').toLowerCase().includes(query))) && (statusFilter === 'all' || (statusFilter === 'in_transit' ? (liveStatus === statusMeta.in_transit) : vehicle.status === statusFilter)) && (typeFilter === 'all' || category(vehicle) === typeFilter)
  })
